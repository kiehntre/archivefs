...
This is a short, plain statement of how EmuWiz handles cheats and mods.
For the fuller technical design behind it, see
[`docs/CHEATS_MODS_SAFETY.md`](CHEATS_MODS_SAFETY.md).

## You're in charge

EmuWiz treats you as a capable adult who can decide what to do with your
...
Where EmuWiz checks content for structural safety, that check runs on
your machine. Filenames, file contents, hashes, scan results, and metadata
about your files and library are never sent to EmuWiz's developers or
...
EmuWiz may inspect a file. It does not run one. Executables, scripts,
...
Every cheat or mod source EmuWiz can represent falls into one of three
states:

- **Trusted** - a source EmuWiz's developers have reviewed: known
  origin, known format, and (where applicable) enforced download limits
  and integrity checking.
- **Unverified** - EmuWiz hasn't reviewed this source. Community
...
- **Blocked** - a concrete technical problem, not a judgment call: an
  ...
  EmuWiz simply cannot inspect safely.
...
EmuWiz is not antivirus software. Its structural checks look for a
...
will not make unsafe files safe - it will only stop EmuWiz checking them.
...
cheats, patches, mods, or other files you bring into EmuWiz. EmuWiz
...
EmuWiz must not be used to bypass copy protection, licensing systems, or
