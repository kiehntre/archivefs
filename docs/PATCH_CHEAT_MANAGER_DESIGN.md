...
The table above is a roadmap, not a claim that those profiles or formats are already supported. PCSX2 is the first implemented read-only candidate-discovery and metadata slice. RetroArch is next, after the PCSX2-specific plan, candidate, identity, and destination types are extracted behind the read-only adapter interface. All further adapters are deferred and must pass a shared contract suite plus adapter-specific fixtures before their names may be shown as supported.

The current code deliberately stops short of the proposed generic trait: `ReadOnlyPcsx2Adapter` owns candidate discovery, while orchestration, matching, the advisory plan's candidate list, and `HypotheticalPnachDestination` still contain PCSX2-specific types. This is acceptable for the first slice but is the exact extraction seam for RetroArch. The next adapter work must introduce emulator-neutral adapter/candidate/capability/destination descriptors and move serial/CRC normalization, PNACH naming, and candidate-relative destination calculation behind the PCSX2 implementation. It must not add RetroArch by copying the PCSX2 conditionals into generic planning.
...
The current Phase 1 filesystem layout is smaller than the target layout: `patch_manager/mod.rs` contains source constants, parsing, catalogue evidence, matching, and advisory planning; `patch_manager/retrieval.rs` contains the fixed-endpoint HTTPS boundary; and `patch_manager/pcsx2.rs` contains read-only standard-path candidate discovery. This avoids empty speculative modules. Before RetroArch, these responsibilities should be extracted into emulator-neutral matching/planning and an `adapters/pcsx2` implementation of the read-only trait. The manifest, backup, transaction, audit persistence, artifact inspection, mutation-adapter methods, other emulator modules, GUI state/rendering, and all install/update/remove/rollback CLI formatting remain absent. Listing later modules here reserves boundaries; it does not authorize scaffolding or implementation in Phase 1.
