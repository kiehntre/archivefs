...
The table above is a roadmap, not a claim that those profiles or formats are already supported. PCSX2 is the first implemented read-only candidate-discovery and metadata slice. **RetroArch is not implemented.** A provisional, narrowly-scoped neutral adapter extraction now sits inside what is still a PCSX2-specific preview pipeline - described precisely below - and it does not by itself make RetroArch (or any other adapter) ready to add.
...
Known RetroArch gaps this extraction does not attempt to close:

- **Profiles**: RetroArch needs its own versioned discovery/format/identity profile (per this document's "Before any adapter gains mutation support" checklist) - not sketched here.
- **Multiple roots per installation**: `InstallationCandidate` still has exactly one `data_root`. RetroArch needs several purpose-tagged roots (cores, cheats, system) per installation, which the current shape cannot express without a further, separately reviewed change.
- **Core inventory**: PCSX2 has no notion of a "core"; discovering which RetroArch cores are actually present is unaddressed.
- **Core ambiguity**: choosing among multiple cores that could run the same content (RetroArch-specific; PCSX2 has no analogue) is unaddressed - `AdvisoryDisposition::AmbiguousInstallationCandidates` covers ambiguous *installations*, not ambiguous *cores* within one installation.
...
The current Phase 1 filesystem layout is still smaller than the target layout, but now includes the neutral adapter boundary: `patch_manager/mod.rs` contains source constants, parsing, catalogue evidence, PCSX2-specific orchestration (platform filtering, probable/uncertain heuristics, ambiguity, plan/plan-ID assembly), and advisory planning; `patch_manager/adapter.rs` contains the neutral `EmulatorAdapter` trait and its supporting types (`AdapterId`, `AdapterCapabilities`, `InstallationCandidate`, `AdapterIdentityEvidence`, `HypotheticalDestination`, `DiscoveryConfidence`); `patch_manager/matching.rs` contains the one small neutral exact-identity-evidence comparison function; `patch_manager/retrieval.rs` contains the fixed-endpoint HTTPS boundary; and `patch_manager/pcsx2.rs` contains read-only standard-path candidate discovery plus the first (and only) `EmulatorAdapter` implementation, including all PCSX2-only serial/CRC normalization, PNACH filename parsing, and destination calculation. This still avoids empty speculative modules and an `adapters/` subdirectory - one additional adapter does not yet justify that additional layer. Before RetroArch, the still-PCSX2-specific orchestration in `mod.rs` (platform filtering and the probable/uncertain heuristics) would need its own review to become adapter-parameterized, alongside RetroArch's explicit-root/multi-root/core-selection profile. The manifest, backup, transaction, audit persistence, artifact inspection, mutation-adapter methods, other emulator modules, GUI state/rendering, and all install/update/remove/rollback CLI formatting remain absent. Listing later modules here reserves boundaries; it does not authorize scaffolding or implementation in Phase 1.
