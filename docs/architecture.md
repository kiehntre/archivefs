...
`library-status` shows the persistent library database's health and counts. `database-check` is the separate safety-oriented diagnostic: it uses explicit SQLite read-only flags, never creates the database or its parent, never migrates, and reports sidecars, rollback-journal header state, recovery-required SQLite results, and stable error categories. `health`, `library-list`, `library-find`, source/alias listings, library-view planning, RetroArch previews, and ordinary GUI catalogue loading also use the explicit read-only open. Scan and mutation commands retain the migration-capable read-write path and SQLite's rollback durability. The GUI keeps scan/source worker handles through normal shutdown and will not replace an active scan, while `SIGKILL` and power loss remain outside application cleanup. Recovery remains a documented, copy-first manual workflow; see [`DATABASE_RECOVERY.md`](DATABASE_RECOVERY.md).
...
That plan now embeds `patch_manager::retroarch_inventory`'s independently versioned, bounded read-only view of supported artifacts already present. It reuses the same environment, expected destinations, playlist/core evidence, byte-safe path rendering, and capability-limited filesystem abstraction; `.cht` files receive bounded metadata-only parsing while soft-patch bodies are never read. The inventory adds no executor or mutation capability and is documented in [`docs/RETROARCH_ARTIFACT_INVENTORY.md`](RETROARCH_ARTIFACT_INVENTORY.md).
